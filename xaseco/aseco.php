...
class Aseco {
	...
	function connect() {
		...
		if ($this->server->ip && $this->server->port && $this->server->login && $this->server->pass) {
			...
			// // log console message
			// $this->console("Trying to authenticate with login '{1}' and password '{2}'",
			//                $this->server->login, $this->server->pass);

			// log console message
			$this->console("Trying to authenticate with TMNF Server...");
...
disabled */
	...
	/**
	 * Check out who won the current track and increment their wins by one.
	 */
