...
function ldb_connect($aseco) {
	...
	// log status message
	// $aseco->console("[LocalDB] Trying to connect to MySQL server on '{1}' with database '{2}'",
	//                 $ldb_settings['mysql']['host'], $ldb_settings['mysql']['database']);
	$aseco->console("[LocalDB] Trying to connect to database server...");
		...
		if ($cur_record === false || ($aseco->server->gameinfo->mode == Gameinfo::STNT ?
			...
			if ($cur_rank != -1) {  // player has a record in topXX already
				...
				if ($cur_rank > $i) {
					...
					// do a player improved their LR rank message
					...
					if ($diff == 0) {
						// do a player equaled their record message
						...
						// do a player secured their record message
