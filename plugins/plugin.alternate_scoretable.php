 ...
 * http://www.undef.name/XAseco1/Alternate-Scoretable.php
 *
 * ----------------------------------------------------------------------------------
 * Author:		undef.de
 * Version:		0.9.2
...
function ast_onSync ($aseco) {
	...
	if ( defined('XASECO_VERSION') ) {
		...
		if ( version_compare($version, $xaseco_min_version, '<') ) {
			trigger_error('[plugin.alternate_scoretable.php] Not supported XAseco version ('. XASECO_VERSION .')! Please update to min. version '. $xaseco_min_version .'!', E_USER_ERROR);
		}
	}
	else {
		trigger_error('[plugin.alternate_scoretable.php] Can not identify the System, "XASECO_VERSION" is unset! This plugin runs only with XAseco/'. $xaseco_min_version .'+', E_USER_ERROR);
	}
	...
	$ast_config['NUMBER_FORMAT'][0] = strtolower($ast_config['NUMBER_FORMAT'][0]);
	$ast_config['NumberFormat'] = array(
		'english'	=> array(
			'decimal_sep'	=> '.',
			'thousands_sep'	=> ',',
		),
...
function ast_showScoretable ($aseco, $caller = false, $timeout = 0, $display_close = true, $page = 0) {
	...
	// Display Previous/Next-Buttons, but not in Team-Mode
	...
	if ($aseco->server->gameinfo->mode == Gameinfo::TEAM) {
		...
		if ( ($aseco->server->gameinfo->mode == Gameinfo::RNDS) || ($aseco->server->gameinfo->mode == Gameinfo::LAPS) || ($aseco->server->gameinfo->mode == Gameinfo::STNT) || ($aseco->server->gameinfo->mode == Gameinfo::CUP) ) {
...
function ast_scoretableDefault ($caller = false, $page = 0) {
	...
	if (count($ast_ranks) > 0) {
		...
		for ($i = ($page * 20); $i < (($page * 20) + 20); $i ++) {
			...
			$xml .= '<label posn="29.9 '. ($position - 0.6) .' 0.14" sizen="2.5 2.4" halign="right" textsize="1" scale="0.9" textcolor="'. $text_color .'" text="'. number_format((int)$ast_config['PLAYERS'][$pos['Login']]['reached_finish'], 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';
			...
			if ( ($aseco->server->gameinfo->mode == Gameinfo::RNDS) || ($aseco->server->gameinfo->mode == Gameinfo::LAPS) || ($aseco->server->gameinfo->mode == Gameinfo::STNT) || ($aseco->server->gameinfo->mode == Gameinfo::CUP) ) {
				...
				else if ($aseco->server->gameinfo->mode == Gameinfo::STNT) {
					$score = $pos['Score'];
				}
			...
			// Ladderscore of Player
			$xml .= '<label posn="60.6 '. ($position - 0.6) .' 0.14" sizen="6.2 2.4" halign="right" textsize="1" scale="0.9" textcolor="'. $text_color .'" text="'. number_format((int)$ast_config['PLAYERS'][$pos['Login']]['ladderscore'], 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';

			// Ladderrank of Player
			$xml .= '<label posn="67.3 '. ($position - 0.6) .' 0.14" sizen="6.5 2.4" halign="right" textsize="1" scale="0.9" textcolor="'. $text_color .'" text="'. number_format((int)$ast_config['PLAYERS'][$pos['Login']]['ladderrank'], 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';
...
//			$xml .= '<label posn="11.8 -3.65 0.12" sizen="5 3" halign="left" textsize="1" scale="0.8" text="$FFFFIN."/>';
		...
		if ( ($aseco->server->gameinfo->mode == Gameinfo::RNDS) || ($aseco->server->gameinfo->mode == Gameinfo::CUP) ) {
		...
		else if ( ($aseco->server->gameinfo->mode == Gameinfo::TA) || ($aseco->server->gameinfo->mode == Gameinfo::TEAM) ) {
...
//		}
		...
		$xml .= '<label posn="9 -15.5 0.12" sizen="25.5 1.5" halign="left" textsize="1" scale="0.9" text="$FFF'. number_format((int)$target->ladderrank, 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';

		// Ladderscore of Player
		$xml .= '<label posn="0 -17.5 0.12" sizen="8 1.5" halign="left" textsize="1" scale="0.9" text="$FFFLadder-Score"/>';
		$xml .= '<label posn="9 -17.5 0.12" sizen="25.5 1.5" halign="left" textsize="1" scale="0.9" text="$FFF'. number_format((int)$target->ladderscore, 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';


		// NbrMatchWins of Player
		$xml .= '<label posn="0 -20.5 0.12" sizen="8 1.5" halign="left" textsize="1" scale="0.9" text="$FFFTotal Won"/>';
		$xml .= '<label posn="9 -20.5 0.12" sizen="25.5 1.5" halign="left" textsize="1" scale="0.9" text="$FFF'.  number_format($ast_config['PLAYERS'][$target->login]['match_wins'], 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';

		// NbrMatchDraws of Player
		$xml .= '<label posn="0 -22.5 0.12" sizen="8 1.5" halign="left" textsize="1" scale="0.9" text="$FFFTotal Draws"/>';
		$xml .= '<label posn="9 -22.5 0.12" sizen="25.5 1.5" halign="left" textsize="1" scale="0.9" text="$FFF'. number_format($ast_config['PLAYERS'][$target->login]['match_draws'], 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';

		// NbrMatchLosses of Player
		$xml .= '<label posn="0 -24.5 0.12" sizen="8 1.5" halign="left" textsize="1" scale="0.9" text="$FFFTotal Losses"/>';
		$xml .= '<label posn="9 -24.5 0.12" sizen="25.5 1.5" halign="left" textsize="1" scale="0.9" text="$FFF'. number_format($ast_config['PLAYERS'][$target->login]['match_losses'], 0, $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['decimal_sep'], $ast_config['NumberFormat'][$ast_config['NUMBER_FORMAT'][0]]['thousands_sep']) .'"/>';
...
function ast_onPlayerFinish ($aseco, $finish_item) {
	...
	if ( ($aseco->server->gameinfo->mode != Gameinfo::TEAM) && ($ast_config['SHOW_AT_FINISH'][0] == true) && ($ast_config['PLAYER_PLACES_WIDGET'][0]['ENABLED'][0] == true) ) {
		ast_getCurrentRanking();
	}
	...
	if ($ast_config['PLAYER_PLACES_WIDGET'][0]['ENABLED'][0] == true) {
		...
		$show = false;
		$addition = '';
		if ( ($aseco->server->gameinfo->mode == Gameinfo::TA) || ($aseco->server->gameinfo->mode == Gameinfo::STNT) ) {
				...
				if ($finish_item->player->login == $pos['Login']) {
					if ( ($ast_config['PLAYERS'][$pos['Login']]['last_rank'] == -1) || (($pos['Rank'] != -1) && ($pos['Rank'] < $ast_config['PLAYERS'][$pos['Login']]['last_rank'])) ) {
						...
						$addition .= '<label posn="5.5 -2.25 0.03" sizen="65 4.5" valign="center" textsize="'. $ast_config['PLAYER_PLACES_WIDGET'][0]['TEXTSIZE'][0] .'" textcolor="'. $ast_config['PLAYER_PLACES_WIDGET'][0]['TEXTCOLOR'][0]['DEFAULT'][0] .'F" text="$SPlayer $'. $ast_config['PLAYER_PLACES_WIDGET'][0]['TEXTCOLOR'][0]['HIGHLITE'][0] . stripColors($finish_item->player->nickname) .'$Z$S reached the '. $pos['Rank'] .'. Place!"/>';
						  ...
						  ( ($ast_config['PLAYERS'][$pos['Login']]['last_finishscore'] > 0) && ($pos['BestTime'] < $ast_config['PLAYERS'][$pos['Login']]['last_finishscore']) && ($aseco->server->gameinfo->mode != Gameinfo::STNT) )
							||
						  ( ($ast_config['PLAYERS'][$pos['Login']]['last_finishscore'] > 0) && ($pos['Score'] > $ast_config['PLAYERS'][$pos['Login']]['last_finishscore']) && ($aseco->server->gameinfo->mode == Gameinfo::STNT) )
						) {
	...
	if ($ast_config['SHOW_AT_FINISH'][0] == true) {
		...
		if ( ($finish_item->challenge->laprace == false) && ($aseco->server->gameinfo->mode != Gameinfo::RNDS) && ($aseco->server->gameinfo->mode != Gameinfo::TEAM) && ($aseco->server->gameinfo->mode != Gameinfo::CUP) ) {
...
function ast_onCheckpoint ($aseco, $checkpt) {
	...
	// Only work at 'Laps'
	if ( ($aseco->server->gameinfo->mode == Gameinfo::LAPS) && ($ast_config['Challenge']['NbCheckpoints'] !== false) ) {
		// If one Player has finish a round, refresh current Ranking
...
function ast_onEndRound ($aseco) {
	...
	if ( ($ast_config['SHOW_AT_FINISH'][0] == true) && ($ast_config['WarmUpPhase'] != true) ) {
		...
		// Display Scoretable
		if ( ($aseco->server->gameinfo->mode == Gameinfo::RNDS) || ($aseco->server->gameinfo->mode == Gameinfo::TEAM) || ($aseco->server->gameinfo->mode == Gameinfo::CUP) ) {
